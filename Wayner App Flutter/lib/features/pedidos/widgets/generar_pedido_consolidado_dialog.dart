...
class GenerarPedidoConsolidadoDialog extends StatefulWidget {
  ...
  const GenerarPedidoConsolidadoDialog({super.key, required this.proveedor});
...
class _GenerarPedidoConsolidadoDialogState
  ...
  // 🔥 NUEVO: Funciones para agregar productos al consolidado
  Future<void> _agregarProducto() async {
    ...
    if (productoSeleccionado != null) {
      _solicitarCantidadYAgregar(productoSeleccionado, items);
    }
  }

  Future<void> _solicitarCantidadYAgregar(
    ...
    if (confirmar == true) {
      ...
      // Tomamos el pedido_id del primer ítem del consolidado como destino del nuevo producto
      final targetPedidoId =
          int.tryParse(itemsActuales.first["pedido_id"].toString()) ?? 0;
      ...
      try {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                "✅ Producto agregado correctamente a la Orden #$targetPedidoId",
              ),
            ),
          );
          _cargarTodo();
                    ...
                    return ListTile(
                      ...
                      onTap: () async {
                        ...
                        try {
                          ...
                          _cargarTodo(); // Recargar consolidado
                        } catch (e) {}
                      },
                    );
                  },
  ...
  String _construirTextoComoPDF(List<dynamic> items) {
    ...
    // Obtenemos todos los pedidos involucrados para listarlos en el texto
  ...
  Future<void> _compartirUnificado(
    ...
    final xFile = XFile.fromData(
      bytes,
      name: 'Consolidado_${widget.proveedor}.pdf',
    ...
    try {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Órdenes actualizadas y notificadas en sistema"),
          ),
        );
    } catch (e) {
      if (mounted)
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: isLoading
            ...
            : Column(
                children: [
                  // 🔥 TÍTULO CON BOTÓN DE AGREGAR PRODUCTO (+)
                  Row(
                  ...
                  // 🔥 INDICADOR DE ESTADO (Estado: BORRADOR)
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Container(
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: const Text("Compartir PDF Consolidado y Texto"),
                      onPressed: items.isEmpty
                          ? null
                          : () {
                              FocusScope.of(context).unfocus();
                              _compartirUnificado(items, textoPlanoFinal);
                            },
                    ),
                  ),
